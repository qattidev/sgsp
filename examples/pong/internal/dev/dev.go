...
func Load(dir string) (Material, error) {
	...
	if _, err := leaf.Verify(x509.VerifyOptions{Roots: roots, DNSName: "localhost"}); err != nil {
		return Material{}, fmt.Errorf("development certificate invalid or expired: %w; stop all Pong processes, remove %s, and restart the server", err, path)
	}
...
func generate(dir, path string) error {
	...
	now := time.Now()
	ca := &x509.Certificate{SerialNumber: big.NewInt(1), Subject: pkix.Name{CommonName: "SGSP Pong Development CA"},
