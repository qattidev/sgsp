...
start_action() {
  ...
  (
    cd "$checkout"
    exec env GOCACHE="$go_cache_dir" go run ./examples/action "$@"
  ) > "$log" 2>&1 &
  server_pids+=("$!")
  started_pid=$!
}
...
wait_for_ready "$direct_pid" "$direct_server_log" 'action owner listening on'
...
wait_for_ready "$owner_a_pid" "$owner_a_log" 'action owner listening on'
...
wait_for_ready "$owner_b_pid" "$owner_b_log" 'action owner listening on'
...
wait_for_ready "$bootstrap_pid" "$bootstrap_log" 'action bootstrap listening on'
